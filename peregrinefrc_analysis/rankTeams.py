...
def make_team_dataframe(
    ...
    for team in counts:
        ...
        for i, _ in enumerate(count_names):
            for j, _ in enumerate(count_functions):
                stats = get_count_stats(counts[team][j])
                if j <= 3:
                    beforeDeath += stats.average * points[j]
                else:
                    aftrDeath += stats.average * points[j]
        
        row.extend([beforeDeath, beforeDeath + aftrDeath])
        data.append(row)

    columns = [
        f"{i} {j}" for i in count_names for j in [" rank score"]
    ]
